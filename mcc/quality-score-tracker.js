...
* Extended by Tibbe van Asten
* © 2016 Martin Roettgerding, Bloofusion Germany GmbH.
* www.ppc-epiphany.com/qstracker/latest
*
* Description:
*  - URL: https://adsscripts.com/nl/scripts/google-ads-scripts/report-qs-mcc
*
* How to use it:
*  - Run script daily
  ...
  */
  var config = {
    ...
    */
    "useClientFolder" : 1,
    /*
    * To select only QS for keywords in campaigns with a name containing the following string.
    */
    "campaignNameFilter" : "",
  }
  ...
  while (accountIterator.hasNext()) {
    ...
    MccApp.select(account);
  	trackQS(config, account);
  }
}

function trackQS(config, account){
  var version = "3.0";

  if(config['useHours']) var dateString = Utilities.formatDate(new Date(), account.getTimeZone(), "yyyy-MM-dd HH:mm");
  else var dateString = Utilities.formatDate(new Date(), account.getTimeZone(), "yyyy-MM-dd");
  ...
  if(config['campaignNameFilter'] != "") awql += " AND CampaignName DOES_NOT_CONTAIN '" + config['campaignNameFilter'] + "'";
